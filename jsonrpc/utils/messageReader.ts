...
let CR: number = textEncoder.encode("\r")[0];
let LF: number = textEncoder.encode("\n")[0];
let CRLF: string = '\r\n';
...
class MessageBuffer {
	...
	private buffer: Deno.Buffer;

	constructor(encoding: string = 'utf8') {
		this.encoding = encoding;
		this.index = 0;
		this.buffer = new Deno.Buffer(new Uint8Array(DefaultSize));
	}

	public append(chunk: Deno.Buffer | String): void {
		var toAppend: Deno.Buffer = <Deno.Buffer>chunk;
